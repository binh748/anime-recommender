...
def create_soup_selenium(url, driver=webdriver.Chrome(chromedriver)):
    """Creates a BeautifulSoup object for a given URL by using Selenium to load
    up the webpage in chromedriver and then extracting the HTML."""
    driver.get(url)
...
# WILL LIKELY DELETE SINCE DID NOT END UP USING THIS FUNCTION
# def get_anime_data(mal_ids):
#     jikan = Jikan()
#     anime_data = []
#     counter = 0
#     for mal_id in mal_ids:
#         anime_data.append(jikan.anime(mal_id))
